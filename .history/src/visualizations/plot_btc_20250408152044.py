...
def plot_acf_pacf_returns(series: pd.Series, nlags: int = 30):
    ...
    Parâmetros:
    - series: pd.Series contendo os retornos logarítmicos.
    - nlags: número de defasagens a serem plotadas.
    """
    ...
    # --> Remove valores ausentes para evitar erros / Drop missing values to avoid errors <--

    acf_vals = acf(series, nlags=nlags)
    pacf_vals = pacf(series, nlags=nlags)
    # --> Calcula ACF e PACF / Computes ACF and PACF <--

    lags = list(range(1, nlags + 1))
    acf_vals_filtered = acf_vals[1:]
    pacf_vals_filtered = pacf_vals[1:]
    # --> Remove lag 0 para melhor visualização / Remove lag 0 for better visualization <--

    conf = 1.96 / np.sqrt(len(series))
    # --> Limites de confiança (95%) / Confidence interval (95%) <--
...
def plot_volatility_rolling(df, window=24):
    ...
    - df: pd.DataFrame contendo a coluna 'log_return'.
    - window: int, janela de tempo usada no cálculo da volatilidade / window size for rolling volatility.
    """
    df = df.copy()
    df["volatility_rolling"] = df["log_return"].rolling(window).std()
    df = df.dropna(subset=["volatility_rolling"])
    # --> Calcula a volatilidade com janela deslizante e remove valores nulos / Computes rolling volatility and drops NaNs <--
    ...
    # --> Adiciona linha da volatilidade com preenchimento suave / Adds volatility line with smooth fill <--
    ...
    # --> Estilização e título / Styling and title <--
